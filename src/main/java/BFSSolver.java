...
public class BFSSolver implements MazeSolver {
    ...
    Queue<int[]> porVisitar = new LinkedList<>();
    ...
    public void init(int[][] maze, int[] start, int[] end) {
        ...
        porVisitar.add(start);
        visitadas.add(toKey(start[0], start[1]));
    }
    ...
    public boolean step() {
        ...
        visitadasEnEstePaso.clear();

        if (porVisitar.isEmpty())
            return true;

        int[] current = porVisitar.poll();
        visitadasEnEstePaso.add(current);
        ...
        for (int[] dir : DIRS) {
            int filaVecina = current[0] + dir[0];
            int columnaVecina = current[1] + dir[1];
            long key = toKey(filaVecina, columnaVecina);

            // revisamos si tenemos que visitar la celda vecina
            if (filaVecina >= 0 && filaVecina < maze.length &&
            columnaVecina >= 0 && columnaVecina < maze[0].length &&
                    maze[filaVecina][columnaVecina] == 1 &&
                    !visitadas.contains(key)) {
                porVisitar.add(new int[]{filaVecina, columnaVecina});
                visitadas.add(key);
                padre.put(key, new int[]{current[0], current[1]});
            }
        }

        return false; // ← Reemplacen esto con su implementación
    }
    ...
    public List<int[]> getVisitedThisStep() {
        ...
        // ──────────────────────────────────────────
        return visitadasEnEstePaso; // ← Reemplacen esto
    }
    ...
    public List<int[]> getPath() {
        ...
        return caminoFinal; // ← Reemplacen esto
    }
